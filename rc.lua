...
function jarradTasklistUpdate(w, buttons, label, data, objects)
    ...
    for i, o in ipairs(objects) do
        local cache = data[o]
        local iconBox, windowNameBox, bgBox, m, windowButton
        if cache then
            ...
            windowButton:set_first(iconBox)
            windowButton:set_second(windowNameBox)
            windowButton:set_third(windowControlsBox)
            
            -- And all of this gets a background
            bgBox = wibox.widget.background()
        ...
        if type(bg_image) == "function" then
            bg_image = bg_image(windowNameBox,o,m,objects,i)
        end
...
for s = 1, screen.count() do
    ...
    -- Create the wibox
    mywibox[s] = awful.wibox({above = true,  align = "center", position = "top", screen = s, width = 1200, height = 25 })
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey, "Control", "Alt" }, "c", function() awful.util.spawn(xkill) end),
...
client.connect_signal("manage", function (c)
    ...
    -- Move plasmoids under the mouse (without going offscreen) and size them according to their requirements (specified in _minimum_ size hints)
    -- TODO: Avoid struts (probably not struts per se)

    if 0 and c.class == "Plasma-desktop" and c.type ~= "dock" and c.skip_taskbar then
